...
impl PorkbunDns {
    pub fn new() -> Result<Self> { Ok(Self { key: env::var("PORKBUN_API_KEY")?, secret: env::var("PORKBUN_SECRET_API_KEY")?,
    ...
    fn api(&self, endpoint: &str, body: &serde_json::Value) -> Result<serde_json::Value> {
        let resp: serde_json::Value = self.client
            ...
            .json(body).send()?.json()?;
        if resp["status"] != "SUCCESS" { bail!("Porkbun error: {resp}"); }
        Ok(resp)
    }
    fn get_record_id(&self, domain: &str) -> Result<Option<String>> {
        ...
        Ok(resp["records"].as_array().and_then(|a| a.iter()
            .find(|r| r["name"].as_str() == Some("_acme-challenge") && r["type"].as_str() == Some("TXT"))
